...
color RaytracingPart3::ray_color(
    ...
    if (srec.is_specular) {
        return srec.attenuationSpe.ToColor()
            * ray_color(srec.specular_ray, background, world, lights, depth - 1);
    }
    ...
    return emitted
        + srec.attenuationSpe.ToColor() * rec.mat_ptr->scattering_pdf(r, rec, scattered)
        * ray_color(scattered, background, world, lights, depth - 1) / pdf_val;
}
...
hittable_list RaytracingPart3::cornell_boxSpe() {
    hittable_list objects;

    auto red = make_shared<lambertian>(Spectrum::FromRGB(color(.65, .05, .05).e));
    auto white = make_shared<lambertian>(Spectrum::FromRGB(color(.73, .73, .73).e));
    auto green = make_shared<lambertian>(Spectrum::FromRGB(color(.12, .45, .15).e));
    auto light = make_shared<diffuse_light>(Spectrum::FromRGB(color(15, 15, 15).e));
