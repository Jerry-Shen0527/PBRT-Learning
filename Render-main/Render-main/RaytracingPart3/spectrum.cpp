...
inline CoefficientSpectrum<nSpectrumSamples> Pow(
    const CoefficientSpectrum<nSpectrumSamples>& s, double e) {
...
int FindInterval(int size, const Predicate& pred) {
    ...
    return check::Clamp(first - 1, 0, size - 2);
}

inline RGBSpectrum Lerp(double t, const RGBSpectrum& s1, const RGBSpectrum& s2) {
    return (1 - t) * s1 + t * s2;
}

inline SampledSpectrum Lerp(double t, const SampledSpectrum& s1,
...
double AverageSpectrumSamples(const double* lambda, const double* vals, int n,
    double lambdaStart, double lambdaEnd) {
    for (int i = 0; i < n - 1; ++i) check::CHECK_GT(lambda[i + 1], lambda[i]);
    check::CHECK_LT(lambdaStart, lambdaEnd);
    ...
    check::CHECK_LT(i + 1, n);

    // Loop over wavelength sample segments and add contributions
    auto interp = [lambda, vals](double w, int i) {
        return check::Lerp((w - lambda[i]) / (lambda[i + 1] - lambda[i]), vals[i],
            vals[i + 1]);
    };
...
double InterpolateSpectrumSamples(const double* lambda, const double* vals, int n,
    double l) {
    for (int i = 0; i < n - 1; ++i) check::CHECK_GT(lambda[i + 1], lambda[i]);
    if (l <= lambda[0]) return vals[0];
    if (l >= lambda[n - 1]) return vals[n - 1];
    int offset = FindInterval(n, [&](int index) { return lambda[index] <= l; });
    check::CHECK(l >= lambda[offset] && l <= lambda[offset + 1]);
    double t = (l - lambda[offset]) / (lambda[offset + 1] - lambda[offset]);
    return check::Lerp(t, vals[offset], vals[offset + 1]);
}
...
void Blackbody(const double* lambda, int n, double T, double* Le) {
    if (T <= 0) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        double l = lambda[i] * 1e-9;
        double lambda5 = (l * l) * (l * l) * l;
        Le[i] = (2 * h * c * c) /
            (lambda5 * (std::exp((h * c) / (l * kb * T)) - 1));
        check::CHECK(!std::isnan(Le[i]));
    }
}

void BlackbodyNormalized(const double* lambda, int n, double T, double* Le) {
    Blackbody(lambda, n, T, Le);
    // Normalize _Le_ values based on maximum blackbody radiance
    double lambdaMax = 2.8977721e-3 / T * 1e9;
    double maxL;
...
void ResampleLinearSpectrum(const double* lambdaIn, const double* vIn, int nIn,
    ...
    double* vOut) {
    check::CHECK_GE(nOut, 2);
    for (int i = 0; i < nIn - 1; ++i) check::CHECK_GT(lambdaIn[i + 1], lambdaIn[i]);
    check::CHECK_LT(lambdaMin, lambdaMax);

    // Spacing between samples in the output distribution.
    double delta = (lambdaMax - lambdaMin) / (nOut - 1);
    ...
    auto lambdaInClamped = [&](int index) {
        check::CHECK(index >= -1 && index <= nIn);
        if (index == -1) {
            check::CHECK_LT(lambdaMin - delta, lambdaIn[0]);
            return lambdaMin - delta;
        }
        else if (index == nIn) {
            check::CHECK_GT(lambdaMax + delta, lambdaIn[nIn - 1]);
    ...
    auto vInClamped = [&](int index) {
        check::CHECK(index >= -1 && index <= nIn);
        return vIn[check::Clamp(index, 0, nIn - 1)];
    };

    // Helper that upsamples ors downsample the given SPD at the given
    // wavelength lambda.
    auto resample = [&](double lambda) -> double {
        ...
        if (end - start == 2 && lambdaInClamped(start) <= lambda - delta &&
            ...
            // lambda.
            double t = (lambda - lambdaInClamped(start)) /
                (lambdaInClamped(end) - lambdaInClamped(start));
            check::CHECK(t >= 0 && t <= 1);
            return check::Lerp(t, vInClamped(start), vInClamped(end));
        }
        else {
    ...
    for (int outOffset = 0; outOffset < nOut; ++outOffset) {
        ...
        double lambda =
            check::Lerp(double(outOffset) / (nOut - 1), lambdaMin, lambdaMax);
